...
class HetMatchDecoder(nn.Module):
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, d_model, d_k, d_v, dropout=0.1, num_n_type=1,
                 num_e_type=1):
        super().__init__()
    ...
    def _compute_QKV_by_etype(self, q, k, v, seq_etype):
        for i, etype in enumerate(seq_etype.unique()):
            ...
            if i == 0:
                ...
                Q += self.w_qs[etype](q * msk)
                K += self.w_ks[etype](k * msk)
                V += self.w_vs[etype](k * msk)
        return Q, K, V
    
    def forward(self, q, k, v, seq_etype=None, seq_utype=None, mask=None):
        """
        params:
            q:    [B, N, D] of src
            k, v: [B, N, D] of ngh
        ...
        q = q.view(sz_b, len_e, n_head, d_k) # (b, N, n_head, D)
...
class TimeEncode(nn.Module):
    def __init__(self, expand_dim):
        super().__init__()
        time_dim = expand_dim
        self.basis_freq = nn.Parameter((torch.from_numpy(
...
class Transfer(nn.Module):
    ...
    def __init__(self, num_n_type, num_e_type, n_dim, e_dim=32,
                 e_type_feat=None):
        super().__init__()
        ...
        if e_type_feat is not None:
            self.edge_trans = nn.Embedding.from_pretrained(e_type_feat,
                                                           padding_idx=0)
        else:
    ...
    def forward(self, src, nghs, seq_utype, seq_vtype, seq_etype):
        ...
        # [B, N, De]
        q = torch.sum(q * u_trans,
                      dim=-1,
                      keepdim=True) * r_trans + self.resize(q, self.e_dim)
        k = torch.sum(k * v_trans,
                      dim=-1,
                      keepdim=True) * r_trans + self.resize(k, self.e_dim)
        return q, k
    
    def resize(self, e, dim):
        # e:[B, N, D]
        if e.size(-1) >= dim:
            res = e[:, :, :dim]
        else:
            e_padd = torch.zeros(e.size(0),
                                 e.size(1),
                                 dim-e.size(2)).to(e.device)
...
class AttnModel(nn.Module):
    ...
    def __init__(self, feat_dim, edge_dim, time_dim, transfer, n_head=2,
                 dropout=0.1, num_n_type=1, num_e_type=1):
        """
        args:
    ...
    def forward(self, src, src_t, seq, seq_t, seq_e, seq_etype, seq_utype,
                seq_vtype, mask):
        ...
        args:
            ...
            seq_etype: long Tensor of shape [B, N], value in (0, 1, ...), 0 is default
            seq_utype: long Tensor of shape [B, N], value in (0, 1, ...), 0 is default
            seq_vtype: long Tensor of shape [B, N], value in (0, 1, ...), 0 is default
            mask: boolean Tensor of shape [B, N], where the true value indicates a null value in the sequence
        returns:
        ...
        # add & norm
        q = self.norm1(q)
        k = self.norm2(k + self.resize(seq_e, self.feat_dim))
